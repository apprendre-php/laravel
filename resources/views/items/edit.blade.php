...
@section('content')
    ...
    <form method="post" action="{{ route('items.update', $item) }}">
        ...
        <div class="mt-4">
            ...
            <x-input id="name" type="text" name="name" :value="old('name', $item->name)" required autofocus />
        </div>
        <div class="mt-4">
            <label class="block uppercase tracking-wide text-xs font-bold mb-2" for="thumbnail">
                Miniature
            </label>
            <x-input id="thumbnail" type="text" name="thumbnail" :value="old('thumbnail', $item->thumbnail)" required />
        </div>
        <div class="mt-4">
            <label class="block uppercase tracking-wide text-xs font-bold mb-2" for="price">
                Prix
            </label>
            <x-input id="price" type="text" name="price" :value="old('price', $item->price)" required />
        </div>
        <div class="mt-4">
            <label class="block uppercase tracking-wide text-xs font-bold mb-2" for="quantity">
                Quantité
            </label>
            <x-input id="quantity" type="text" name="quantity" :value="old('quantity', $item->quantity)" required />
        </div>
        <div class="mt-4">
            <label class="block uppercase tracking-wide text-xs font-bold mb-2" for="description">
                Description
            </label>
            <x-input id="description" type="text" name="description" :value="old('description', $item->description)" required />
        </div>
        <div class="mt-4">
